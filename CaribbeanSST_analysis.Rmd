...
had_gps <- had_gps[, -c(5:7)] # remove columns for hour, minute, and seconds (we won't use these)
...
save(had_gps, file = "data/HadISST/HadISST_reefs_SST_update.Rdata") # to save file as Rdata 
...
HadISST_SST_plot <- ggplot() +
  ...
  geom_smooth(data = HADI_year_avg[-151,], method = "gam", aes(x = year, y = mean), colour = "black", group = 1, size = 1, se = FALSE, formula = y ~ s(x, bs = "cc")) + # smoothed mean temp  
  #ggtitle("HadISST (1870 - 2019)") +
...
#  geom_line(data = HADI_year_avg[-151,], aes(x = year, y = max), colour = "#d6604d", group = 1, size = 0.5) +
#  geom_line(data = HADI_year_avg[-151,], aes(x = year, y = min), colour = "#2166ac", group = 1, size = 0.5) + 
#  geom_smooth(data = HADI_year_avg[-151,], method = "glm", aes(x = year, y = mean), colour = "black", group = 1, size #= 1, se = FALSE) + # smoothed mean temp
#  ggtitle("HadISST (1870 - 2019)") +
#  coord_cartesian(xlim = c(1870, 2019)) +
#  ggsave("figures/Fig3_HadISST_SST_annual.pdf", width = 6, height = 4)
...
#regions_cropped <- crop(region, extent(-90, -55, 8, 30)) # crop for the Caribbean region only
