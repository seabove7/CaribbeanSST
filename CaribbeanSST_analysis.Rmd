...
# install.packages("sf", "ncdf4", "raster", "maptools", "rgdal", "ggplot2", "tidyverse", "xts", "RColorBrewer", "tidyr", "dplyr", "fields", "gganimate", "sp", "plotly", "grDevices")
...
gshhs.f.b <- "data/ReefData/gshhs_f.b"
...
shape <- shapefile("data/ReefData/14_001_WCMC008_CoralReefs2018_v4/01_Data/WCMC008_CoralReef2018_Py_v4.shp")
...
had_data <- "data/HadISST/HadISST_sst.nc"
...
```{r custom cluster - had, eval=FALSE, include=FALSE}

## For first time installment of 'doAzureParallel'
# library(devtools)
# install_github("Azure/doAzureParallel")

# Perform parallel calculation on Azure cluster:

# 1. Load the 'doAzureParallel' and 'forEach' libraries
library(doAzureParallel)
library(foreach)

# 2. Load the credentials file (which should be placed in the working directory), to allow R to authenticate with the Azure cluster.
# NOTE - make sure this is not committed to git!
setCredentials("/Users/colleen/Dropbox/Shrubbery_Files/credentials.json") 

# 3. 'Make' the cluster. The cluster defined in 'cluster.json' (named 'shrubbery') already exists, so this should not actually create a cluster instance, just tell R to use the existing cluster, and specify Docker images to use etc.
cluster <- makeCluster("Shrubbery_Files/cluster_had.json")

# 4. Register the cluster to use it for parallel processing
registerDoAzureParallel(cluster)

# 5. Perform the parallel processing:
#   - Use the R 'foreach' library, rather than a regular for loop
#   - Specify 'rbind' to recombine the individual computations into the data frame
#     using 'rbind'. Other combination functions (e.g. 'cbind') can be used if 
#     needed in a different situation
#   - Use the '%dopar%' operator to tell R to run the code inside the foreach
#     statement as parallel processes
#   - Some data types transfer nicely to the cluster (e.g. the reef shape file). Others
#     do not (e.g. the netCDF data brick). In this case, the data brick is read from
#     file on the cluster by each node, rather than trying to 'send it' via R.
#     It is part of the Docker image, and stored in /data/.
#   - Explicitly include any libraries used inside of the foreach loop into '.packages'
#     parameter (libraries used only outside the 'foreach' don't need to be included)

n_iterations <- data_brick@file@nbands
#n_iterations <- 3
had_df <- foreach(i = 1:n_iterations, .combine = 'rbind', .packages = c('raster', 'rgdal')) %dopar% {

  rast <- brick("/data/HadISST_sst.nc")[[i]]
  
  mean_sst_vals <- raster::extract(rast, shapes, fun=mean)
  min_sst_vals <- raster::extract(rast, shapes, fun=min)
  max_sst_vals <- raster::extract(rast, shapes, fun=max)
  weighted_mean <- raster::extract(rast, shapes, weights=TRUE, fun=mean)
  
  df <- data.frame('date' = rast@data@names,
                   'mean' = mean_sst_vals,
                   'min' = min_sst_vals,
                   'max' = max_sst_vals,
...
save(had_df, file = "Users/colleen/Dropbox/Coauthor Publication/Precht_Review/data/HadISST_reefs_poly.Rdata")

```

```{r forloop - had, eval=FALSE, include=FALSE, paged.print=FALSE}
...
save(had_df, file = "data/HadISST/HadISST_reefs.Rdata")
...
path_data <- "data/PathfinderSST/pathfinder_combined_monthly_data.nc"
...
```{r path raster edit and slope calculation, eval=FALSE, include=FALSE}
...
#save(path_slope, file = "data/PathfinderSST/path_slope.Rdata")
#write.csv(path_slope, file = "data/PathfinderSST/path_slope.csv", row.names=FALSE)

# --------- *Plotting occurs down at the bottom of the markdown file* --------- #

```

```{r forloop - path, eval=FALSE, include=FALSE, paged.print=FALSE}
...
# successful code for extracting by polygons goes here ---->
...
## this is the data frame of extracted SST based on polygons
load("data/PathfinderSST/Pathfinder.Rdata")

path_df$mean2 <- round((path_df$mean - 273.15), 2)
path_df$weighted2 <- round((path_df$weighted - 273.15), 2)
path_df$diff2 <-round((path_df$mean - path_df$weighted), 2)
...
NAcol <- colnames(path_poly)[colSums(is.na(path_poly)) > 0] # identify the columns with NA values to pull SST from GPS points instead of polygons
reef_gps <- reef_pt %>% filter(L3 == NAcol) # select only GPS points of polygons missing SST data above
...
#some pixels fall on land. Finding nearest valid pixels
#pixels coordinates
first_day2 <- data_brick2[[1]]

plot(first_day2, col = colr, xlab="Long", ylab="Lat", main=paste0("Pathfinder SST on ",start_date2), colNA="grey", cex.main=0.8, cex.axis=0.8, cex.lab=0.7, ylim = c(8, 30), xlim = c(-95, -55))

pixel_coordinates <- coordinates(first_day2)
#delete coordinates where data is missing (NA)
#first, extract data for all pixels
data <- raster::extract(first_day2, pixel_coordinates)
#now check which ones are NA and delete those entries from the "pixel.coordinates" list
pixel_coordinates <- pixel_coordinates[!is.na(data),]
...
#extract data from the sst brick with the sampling points shifted
path_sst <- raster::extract(data_brick2, ReefPTS_shifted)
#look at it lines are locations; columns are dates
