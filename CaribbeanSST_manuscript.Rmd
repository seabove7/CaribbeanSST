...
eco_gam_slope %>% group_by(ecoregion) %>% filter(year > 1975 & !is.na(value)) %>% filter(value == min(value))


filter()## Plots each individual GAM per ecoregion with highlighted regions of significant warming as calculated above. This plot is not in the manuscipt at present.
ggplot() +
  theme_bw() +
