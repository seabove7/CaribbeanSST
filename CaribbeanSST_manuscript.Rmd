...
path_gam_pred <- predict.gam(path_gam$gam, type = "response", se.fit = TRUE, exclude = s(month)) # predict response-level values from the GAM per year
...
```{r figure S4 comparing SST on reefs between databases}
...
ggsave("figures/Supplemental/Fig4_PathVhadISST.pdf", width = 8, height = 5)
ggsave("figures/Supplemental/Fig4_PathVhadISST.png", width = 8, height = 5)

```

Comparison of monthly SST (°C) recorded in the HadISST and Pathfinder databases from 1982-2019. SST values were regressed against one another for the same time for comparison of the relationship between the databases. Points are depicted by month (cooler months in blue and warmer months in red) and the black line represents the resulting linear regression model (R^2^ = `r round(had_path_lm_sum$adj.r.squared, 2)`; P < 0.001).
...
# Saves the manuscript version of the Pathfinder historic SST
ggplotly(stripes)
  ggsave("figures/Supplemental/FigS10_ClimateStripes.pdf", width = 6, height = 3.3)
  ggsave("figures/Supplemental/FigS10_ClimateStripes.png", width = 6, height = 3.3)
...
plot_freq_pixel <- ggplot(data=frequency_df, aes(x= year, y= total_events)) +
  ...
  geom_line(data = frequency_df2, aes(x = year, y = pred), colour = "red", size = 0.5) +
  geom_line(data = frequency_df3, aes(x = year, y = pred), colour = "green", size = 0.5) +
