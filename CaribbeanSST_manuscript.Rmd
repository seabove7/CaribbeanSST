...
needed_packages <- c("sf", "ncdf4", "ggrepel", "raster", "viridis", "maptools", "rgdal", "ggplot2", "cowplot", "tidyverse", "rnaturalearth", "xts", "RColorBrewer", "tidyr", "dplyr", "binr", "fields", "gganimate", "sp", "mgcv", "plotly", "grDevices", "repmis", "foreach", "kableExtra", "splines", "gridExtra", "Hmisc", "ggpubr", "sjPlot", "broom", "modelr", "performance", "fmsb", "transformr", "gifski", "gapminder") # Specify necessary packages
...
### Combined plots
Figure4 <- ggarrange(path_map, map_slope, labels = c("A", "B"), label_size = 18) +
...
```{r MHW decade values, eval=FALSE, include=FALSE}

## These are not included in the Rmarkdown document but are referenced in the text of the main manuscript
...
freq_decade_summary <- mhw_pixel %>% 
  ...
  group_by(decade) %>% 
  summarise(mean_duration = mean(total_events, na.rm = TRUE))
...
duration_decade_summary <- mhw_pixel %>% 
  ...
  mutate(decade = floor(as.numeric(year) / 10) * 10) %>% 
  group_by(decade) %>% 
  summarise(mean_duration = mean(avg_duration, na.rm = TRUE))
...
rt_decade_summary <- mhw_pixel %>% 
  ...
  mutate(decade = floor(as.numeric(year) / 10) * 10) %>% 
  group_by(decade) %>% 
  summarise(mean_duration = mean(avg_rt, na.rm = TRUE))
...
plot_dur_region <- ggplot(data = mhw_pixel %>% filter(!is.na(avg_duration)), aes(x = year, y = avg_duration)) + 
  theme_bw() +
...
plot_rt_region <- ggplot(data = mhw_pixel %>% filter(!is.na(avg_rt)), aes(x = year, y = avg_rt)) + 
  theme_bw() +
  #theme(panel.grid.major=element_blank(), panel.grid.minor=element_blank(), strip.background = element_blank(), strip.text.x = element_blank(), axis.title.x=element_blank(), axis.text.x=element_blank()) +
...
```{r combine and save MHWs by ecoregion for Fig S9}

## Final combined MHW plot and save it
ggarrange(plot_freq_region, plot_dur_region, plot_rt_region, ncol = 1, labels = "AUTO", heights = c(0.85, 0.75, 0.95)) +
  ggsave("figures/Supplemental/FigS9_MHW_ecoregion_metrics.pdf", width = 12, height = 6)
...
```{r export tables to excel WB, eval=FALSE, include=FALSE}
