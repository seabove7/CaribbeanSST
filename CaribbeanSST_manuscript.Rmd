...
# install.packages("sf", "ncdf4", "ggrepel", "raster", "viridis", "maptools", "rgdal", "ggplot2", "cowplot", "tidyverse", "rnaturalearth", "xts", "RColorBrewer", "tidyr", "dplyr", "binr", "fields", "gganimate", "sp", "mgcv", "plotly", "grDevices", "repmis", "foreach", "kableExtra", "splines", "gridExtra", "Hmisc", "ggpubr")
...
# MHW data for the entire basin (not just where coral reefs are)

#mhw_carib <- read_csv("/Users/colleen/Dropbox/Git/Laura_Files/mhw_carib_backup_july312019.csv")
mhw_carib <- read_csv("/Users/colleen/Dropbox/Git/Laura_Files/mhw_carib.csv")
...
mhw_carib_reefs <- read_csv("mhw_carib_reefs.csv")
...
eco.labs <- c("Bahamas", "E. Carib", "Florida", "Antilles", "Gulf of Mexico", "S. Carib", "SW Carib", "W. Carib")
names(eco.labs) <- c("Bahamian", "Eastern Caribbean", "Floridian", "Greater Antilles", "Gulf of Mexico", "Southern Caribbean", "Southwestern Caribbean", "Western Caribbean")
...
# final plot (panel B in Figure 6)
...
plot_freq_region <- ggplot(frequency_df, aes(x=year, y=total_events)) + 
  theme_bw() +
  theme(panel.grid.major=element_blank(), panel.grid.minor=element_blank(), panel.background=element_blank(), strip.background = element_blank(), panel.border = element_rect(colour="black", fill=NA, size=1), strip.text.x = element_blank(), axis.title.x=element_blank(), axis.text.x=element_blank()) +
...
# final plot (panel B in Figure 6)
plot_dur_pixel <- ggplot(data=duration_df, aes(x= year, y= avg_duration)) +
  theme_bw() +
  theme(panel.grid.major=element_blank(), panel.grid.minor=element_blank(), panel.background=element_blank(), strip.background = element_blank(), panel.border = element_rect(colour = "black", fill=NA, size=1)) + 
  stat_summary(fun.data = mean_sdl, fun.args = list(mult = 1), geom = "errorbar", width = 0) +
  stat_summary(fun = "mean", size = 0.3) +
...
plot_dur_region <- ggplot(duration_df, aes(x=year, y=avg_duration)) + 
  theme_bw() +
  theme(panel.grid.major=element_blank(), panel.grid.minor=element_blank(), panel.background=element_blank(), strip.background = element_blank(), panel.border = element_rect(colour="black", fill=NA, size=1), strip.text.x = element_blank(), axis.title.x=element_blank(), axis.text.x=element_blank()) +
  #geom_point(colour = "grey", alpha = 0.15, position = position_jitter()) +
  stat_summary(fun="mean", geom= "point") +
...
# final plot (panel C in Figure 6)
plot_rt_pixel <- ggplot(data=rt_df, aes(x= year, y= avg_rt)) +
  theme_bw() +
  theme(panel.grid.major=element_blank(), panel.grid.minor=element_blank(), panel.background=element_blank(), strip.background = element_blank(), panel.border = element_rect(colour = "black", fill=NA, size=1)) + 
  stat_summary(fun.data = mean_sdl, fun.args = list(mult = 1), geom = "errorbar", width = 0) +
  stat_summary(fun = "mean", size = 0.3) +
...
plot_rt_region <- ggplot(rt_df, aes(x=year, y=avg_rt)) + 
  theme_bw() +
  theme(panel.grid.major=element_blank(), panel.grid.minor=element_blank(), panel.background=element_blank(), strip.background = element_blank(), panel.border = element_rect(colour="black", fill=NA, size=1), strip.text.x = element_blank(), axis.title.x=element_blank(), axis.text.x=element_blank()) +
  #geom_point(colour = "grey", alpha = 0.15, position = position_jitter()) +
  stat_summary(fun="mean", geom= "point") +
...
## Just looking at return rates here:

annual_rt_avg <- mhw_pixel %>% 
  group_by(year) %>% 
  summarize(mean = round(mean(avg_rt, na.rm = TRUE)))

eco_rt_avg <- mhw_pixel %>% 
  group_by(year, ecoregion) %>% 
  summarize(mean = round(mean(avg_rt, na.rm = TRUE)))


decade_eco_rt_avg <- mhw_pixel %>% 
  select(year, ecoregion, avg_rt) %>% 
  mutate(decade = floor(as.numeric(year) / 10) * 10) %>% 
  group_by(decade, ecoregion) %>% 
  summarize(mean = mean(avg_rt, na.rm = TRUE))

decade_rt_avg <- mhw_pixel %>% 
  select(year, avg_rt) %>% 
  mutate(decade = floor(as.numeric(year) / 10) * 10) %>% 
  group_by(decade) %>% 
  summarize(mean = mean(avg_rt, na.rm = TRUE))
...
```{r combine and save MHW figure 5}
...
freq_decade_summary <- mhw_pixel %>% 
  ...
  mutate(decade = floor(as.numeric(year) / 10) * 10) %>% 
  group_by(decade) %>% 
  summarize(mean_duration = mean(total_events, na.rm = TRUE))
...
rt_decade_summary <- mhw_pixel %>% 
  ...
  mutate(decade = floor(as.numeric(year) / 10) * 10) %>% 
  group_by(decade) %>% 
  summarize(mean_duration = mean(avg_rt, na.rm = TRUE))
...
```{r export tables to excel WB, message=FALSE, warning=FALSE}

### Create workbook
wb <- createWorkbook()

# add 'Table S2' worksheet
addWorksheet(wb, "Table S2 - Caribbean warming") 
writeData(wb, sheet = 1, x = table_S2)
