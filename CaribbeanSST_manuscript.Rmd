...
## And again for 1987–2020 for the rate since inflection:
# apply the function
raster_pval_87 <- calc(data_brick_87, function(x)pval.fun(x, time = time_87))

# convert the raster to a df for plotting
had_pval_87 <- as.data.frame(as(raster_pval_87, "SpatialPixelsDataFrame")) # convert to a raster to dataframe
colnames(had_pval_87) <- c("pval", "x", "y")
had_pval_87$bins <- cut(had_pval_87$pval, breaks = bins, labels = bin_names) # bins the pvales into 6 unique bins for plotting
