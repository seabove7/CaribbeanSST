...
**Caribbean coral reef site locations and ecoregion designation.** The colour of each reef represents the designated ecoregion and n denotes the number of unique reef locations within that ecoregion. Map layer was acquired from Natural Earth ([https://www.naturalearthdata.com](https://www.naturalearthdata.com)).
...
```{r figure SX comparing SST on reefs between databases}
...
ggsave("figures/Supplemental/FigXX_PathVhadISST.pdf", width = 8, height = 5)
ggsave("figures/Supplemental/FigXX_PathVhadISST.png", width = 8, height = 5)
...
# Saves the manuscript version of the Pathfinder historic SST
ggplotly(stripes)
  ggsave("figures/Supplemental/FigS9_ClimateStripes.pdf", width = 6, height = 3.3)
  ggsave("figures/Supplemental/FigS9_ClimateStripes.png", width = 6, height = 3.3)

```

Climate stripe diagram depicting the mean annual temperature recorded on Caribbean coral reefs from 1870 to 2020 based on the HadISST database. Warmer temperatures are depicted in red (maximum annual SST of `r round(max(had_annual_df$mean), 2)`) and cooler annual temperatures are in blue (minimum annual SST of `r round(min(had_annual_df$mean), 2)`). 
...
# Diagnosis of model:
#check_model(pixel_frequency_glm)
...
## Modelling per ecoregion
pixel_duration_eco <- lme4::lmList(log(avg_rt) ~ year | ecoregion, data = mhw_pixel %>% filter(!is.na(avg_rt)))
#summary(pixel_duration_eco)
...
table_1 <- data.frame("Temperature parameter" = c("Caribbean Basin (°C per decade)", "Caribbean Basin (total °C for period)", "Caribbean Reefs (°C per decade)", "Caribbean Reefs (total °C for period)"),
                      ...
                      "HadISST (1994-2020)" = c(had_basin_sub_94_rate, had_basin_sub_94_value, had_reef_sub_94_rate, had_reef_sub_94_value), 
...
# View the completed 
kable(table_1, booktabs = TRUE, row.names = FALSE, col.names = c("Temperature parameter", "HadISST (1871-2020)", "HadISST (1981-2020)", "HadISST (1994-2020)", "Pathfinder (1981-2019)", "Pathfinder (1994-2019)")) %>% 
...
table_s3 <- kable(mhw_mod_out, booktabs = T) %>%
...
```{r table S4 - MHW decade summaries}
...
table_s4 <- kable(mhw_decade_full[-1], booktabs = T) %>%
  kable_styling(font_size = 12, full_width = F) %>% 
...
addWorksheet(wb, "Table S4 - MHW metrics") 
