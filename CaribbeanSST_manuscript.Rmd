...
##### **Citation:**
Bove CB, Mudge L, Bruno JF (2022) A century of warming on Caribbean reefs. PLOS Climate 1(3): e0000002. [https://doi.org/10.1371/journal.pclm.0000002](https://doi.org/10.1371/journal.pclm.0000002)
...
# Dropbox link: https://www.dropbox.com/s/xcem349nbx8x1mg/PathfinderSST_monthly_edit.nc?dl=0
...
had_year_plot <- ggplot() +
  theme_bw() +
  theme(panel.grid.major=element_blank(), panel.grid.minor=element_blank(), panel.background=element_blank(), strip.background = element_blank(), panel.border = element_rect(colour = "black", fill=NA, size=1), text = element_text(size = 14), legend.position = "none", plot.margin = margin(10, 13, 10, 10)) +
  geom_hline(yintercept = mean(HADI_year_avg$mean), colour = "grey", linetype = 2) +
  geom_vline(xintercept = 1988, colour = "grey", linetype = 1) + #Bahamian
  geom_vline(xintercept = 1984, colour = "grey", linetype = 1) + #E Caribbean
  geom_vline(xintercept = 1993, colour = "grey", linetype = 1) + #Floridian
  geom_vline(xintercept = 1986, colour = "grey", linetype = 1) + #G Antilles
  geom_vline(xintercept = 1981, colour = "grey", linetype = 1) + #GOM
  geom_vline(xintercept = 1981, colour = "grey", linetype = 1) + #S Caribbean
  geom_vline(xintercept = 1982, colour = "grey", linetype = 1) + #SW Caribbean
  geom_vline(xintercept = 1999, colour = "grey", linetype = 1) + #W Caribbean
  ...
  #geom_text(aes(x = 1999, y = 24.9, label = "1994"), size = 3) +
  #geom_text(aes(x = 1920, y = 24.9, label = "1915"), size = 3) +
  ...
  coord_cartesian(xlim = c(1870, 2020), expand = FALSE) 
...
ggarrange(had_month_plot, had_year_plot, ncol = 1, labels = c("A", "B"), align = "hv")
  ggsave("figures/Manuscript/Fig2_SST_trends.pdf", dpi = 1200, width = 6, height = 7.5)
  ggsave("figures/Manuscript/Fig2_SST_trends.png", dpi = 1200, width = 6, height = 7.5)
...
```{r plot ecoregion GAM slope significance, fig.height = 8, fig.width = 7}

# make colour column for the different warming periods
eco_gam_slope$col <- factor(ifelse(is.na(eco_gam_slope$value), 'NA', "#fee090"))
...
ggplot() +
  ...
  #geom_rect(data = eco_gam_slope %>% group_by(ecoregion) %>% filter(year > 1975 & !is.na(value)) %>% filter(value == min(value)), aes(ymin = -Inf, ymax = Inf, xmin = year, xmax = Inf), colour = 'black', fill = "#fee090", alpha = 0.2) + 
  geom_rect(data = eco_gam_slope, aes(xmin = year, xmax = year+1, ymin = -Inf, ymax = Inf, fill = col), alpha = 0.4, show.legend = FALSE) +
  scale_fill_manual(values = c("#fee090", "transparent")) +
 ...
 # geom_ribbon(data = ecoregion_sst_annual, aes(x = year, ymin = lowerci, ymax = upperci), colour = "darkgrey", alpha = 0.1, linetype = 1, size = 0.2) + 
  ...
  coord_cartesian(xlim = c(1868, 2022), expand = FALSE) +
  scale_y_continuous(breaks = seq(20, 32, 2), limits = c(19, 32)) +
  facet_wrap(~ ecoregion, ncol = 2, scales = "free_y") +
  #geom_vline(data = eco_gam_slope %>% group_by(ecoregion) %>% filter(year > 1975 & !is.na(value)) %>% filter(value == min(value)), mapping = aes(xintercept = year), colour = "grey30", linetype = 1) +  # just a line for year of resumption of significant warming
  ...
  ggsave("figures/Manuscript/Fig3_Ecoregion_SST_trends.png", width = 7, height = 8)
...
ggarrange(path_month_plot, path_year_plot, ncol = 1, labels = c("A", "B"), align = "hv")
  ggsave("figures/Supplemental/FigS1_Pathfinder_SST_trends.pdf", width = 6, height = 7.5)
  ggsave("figures/Supplemental/FigS1_Pathfinder_SST_trends.png", width = 6, height = 7.5)
...
```{r figure S2 comparing SST on reefs between databases}
...
ggsave("figures/Supplemental/FigS2_PathVhadISST.pdf", width = 8, height = 5)
ggsave("figures/Supplemental/FigS2_PathVhadISST.png", width = 8, height = 5)
...
combo_plot <- ggplot() +
  ...
  ggsave("figures/Supplemental/FigS3_Both_SST_trends.pdf", width = 6, height = 4)
  ggsave("figures/Supplemental/FigS3_Both_SST_trends.png", width = 6, height = 4)
...
![](figures/Supplemental/FigS4_Ecoregion_MapSST.png)
...
had_plot_gif <- ggplot() +
  ...
  #annotate(geom = "text", size = 4, x = 11, y = 24.6, label = "@DrSeaBove", colour = "grey20") +
...
anim_save("HadISST.gif")


## saving a version for presentations
# increase delay on final frame of GIF
animate(had_gif, fps = 10, nframes = 300, end_pause = 30, width = 650, height = 400, renderer = gifski_renderer(loop = FALSE))
anim_save("HadISST_presentations.gif", animation = last_animation())
...
Figure4 <- ggarrange(path_map, map_slope, labels = c("A", "B"))
  ...
  ggsave("figures/Manuscript/Fig4_SST_MHW_map.png", width = 14.5, height = 5)
...
ggarrange(plot_freq_pixel, plot_dur_pixel, plot_rt_pixel, ncol = 3, labels = "AUTO")
  ...
  ggsave("figures/Manuscript/Fig5_MHW_metrics.png", width = 10, height = 3)
...
## Combined p values:
FigureS5 <- ggarrange(path_pval_map, map_p, align = "hv", labels = c("A", "B"))
  ggsave("figures/Supplemental/FigS5_SST_MHW_significance_map.pdf", width = 14.5, height = 5)
  ggsave("figures/Supplemental/FigS5_SST_MHW_significance_map.png", width = 14.5, height = 5)
...
ggarrange(had_map, had_pval_map, labels = c("A", "B"))
  ggsave("figures/Supplemental/FigS6_HadISST_map.pdf", width = 14, height = 5)
  ggsave("figures/Supplemental/FigS6_HadISST_map.png", width = 14, height = 5)

```

**Supplemental Figure 6. A**) Rate of SST change (°C per decade) over the duration of the HadISST database across the Caribbean from 1871 to 2020 (mean slope `r round(mean(had_slope_clipped$sst), 2)` ± `r round(sd(had_slope_clipped$sst), 3)` °C per decade) and **B**) significance of rate of SST change. Grey ocean area was not included in these analyses.
...
ggarrange(had_map_sub, had_pval_map_sub, labels = c("A", "B"))
  ggsave("figures/Supplemental/FigS7_HadISST_subset_map.pdf", width = 14, height = 5)
  ggsave("figures/Supplemental/FigS7_HadISST_subset_map.png", width = 14, height = 5)

```

**Supplemental Figure 7. A**) Rate of SST change (°C per decade) across the Caribbean from 1981 to 2020 (HadISST; mean slope `r round(mean(had_slope_sub_clipped$sst), 2)` ± `r round(sd(had_slope_sub_clipped$sst), 3)` °C per decade) and **B**) significance of rate of SST change. Grey ocean area was not included in these analyses.
...
```{r combine and save MHWs by ecoregion for Fig S8, fig.width = 12, fig.height = 10}
...
ggarrange(plot_freq_region, plot_dur_region, plot_rt_region, plot_int_region, plot_onset_region, plot_totalDays_region, ncol = 1, heights = c(0.85, 0.75, 0.75, 0.75, 0.75, 0.95))
  ggsave("figures/Supplemental/FigS8_MHW_ecoregion_metrics.pdf", width = 16, height = 12)
  ggsave("figures/Supplemental/FigS8_MHW_ecoregion_metrics.png", width = 16, height = 12)
...
Sea surface temperature and visualization code written by [Colleen B. Bove](https://colleenbove.science) and marine heatwave code written by [Laura Mudge](https://lmudge13.github.io/index.html).
