...
data_brickb <- data_brick[[Tstart_indexb:length(nc_times)]]
...
had_sst_long <- gather(had_sst_df, date, sst, X1870.01.16.15.00.00:X2020.12.16.15.00.00) # convert from wide to long format
...
hadplot_markdown <- ggplot() +
  ...
  geom_line(data = had_full2, aes(x = month, y = mean_temp, group = year, colour = year), alpha = 0.6, size = 0.4) +
...
had_plot_gif <- ggplot() +
  ...
  guides(colour = guide_colorbar(frame.colour = "black", ticks.colour = "black")) +
  geom_line(data = had_full2, aes(x = month, y = mean_temp, group = year, colour = year), size = 1) +
...
```{r Pathfinder raster, eval=FALSE, include=FALSE}
...
# save time component
writeRaster(data_brick2, file = "data/Pathfinder/Pathfinder_monthlySST_corrected.nc")
...
## read in the Pathfinder raster
path_data <- "data/Pathfinder/PathfinderSST_UPDATE_monthly_edit2.nc"
data_brick2 <- brick(path_data) # read SST data as rasterbrick
data_brick2 <- crop(data_brick2,extent(Xmin,Xmax,Ymin,Ymax)) # clip rasterbrick for Caribbean region only (see 'spatial subsetting' above)
...
saveRDS(path_gps, file = "data/Pathfinder/Pathfinder_reefs_SST_update2.rds") # to save file as Rdata (I saved as Rdata since it is large)
...
path_full <- readRDS("data/Pathfinder/Pathfinder_reefs_SST_update2.rds")
...
plot_grid(had_map, had_pval_map, labels = c("A", "B")) +
  ggsave("figures/Supplemental_figs/HadISST_map.pdf", width = 14, height = 5)

```


**Figure SXX |** Change in SST ($^\circ$C / decade) over the timespand of the HadISST dataset (`r format.Date(start_date, "%Y")` - `r format.Date(end_date, "%Y")`) and the resulting significance of the regression (**B**). The average temperature change for the entire region is `r round(mean(had_slope_clipped$sst), 2)` ($\pm$ `r round(sd(had_slope_clipped$sst), 3)`). 
...
plot_grid(had_map_sub, had_pval_map_sub, labels = c("A", "B")) +
  ggsave("figures/Supplemental_figs/HadISST_subset_map.pdf", width = 14, height = 5)

```

**Figure SXX |** Change in SST ($^\circ$C / decade) over the subset timespand of the HadISST dataset (subset: `r format.Date(start_dateb, "%Y")` - `r format.Date(end_date, "%Y")`) (**A**) and the resulting significance of the regression (**B**). The average temperature change for the entire region is `r round(mean(had_slope_sub_clipped$sst), 2)` ($\pm$ `r round(sd(had_slope_sub_clipped$sst), 3)`). 
...
path_map <- ggplot() +  
  ...
  ylab("Latitude") +
  xlab("Longitude") +
  ggtitle(paste0("SST slope from ", format.Date(start_dateb, "%Y"), " to ", format.Date(end_date, "%Y")," (Pathfinder)"))
...
```{r ecoregion SST extraction (HadISST), eval=FALSE, include=FALSE}
...
for(e in 1:length(eco_list)) {
  ...
  for(i in 1:raster_brick@data@nlayers) {
...
n## Once completed, saves the resulting dataframe as both a .Rdata object and .csv
# *** Only uncomment these lines if you want to write over existing files ***
...
```{r ecoregion SST extraction (Pathfinder), eval=FALSE, include=FALSE}

#### SST extraction on reefs by ecoregion: this takes a while to run - can try to run remote
raster_brick <- data_brick2

## Create an empty dataframe to populate while running the forloop with the following parameters:
eco_sst_path_df <- data.frame('date' = integer(),
                     'sst' = integer(),
                     'ecoregion' = factor())
...
for(e in 1:length(eco_list)) {
  ...
  for(i in 1:raster_brick@file@nbands) {
    ...
    df <- data.frame('date' = raster@data@names,
                     'sst' = sst_vals)
    df$ecoregion <- rep(ecoregion_name, length(df$date)) # add a ecoregion name column
    eco_sst_path_df <- rbind(eco_sst_path_df, df) # adds current layer SST data to the full dataframe
...
## Once completed, saves the resulting dataframe as both a .Rdata object and .csv
# *** Only uncomment these lines if you want to write over existing files ***
#saveRDS(eco_sst_path_df, file = "data/Pathfinder/Ecoregion_SST/Pathfinder_ecoregion_sst.rds")
#write.csv(eco_sst_path_df, file = "data/Pathfinder/Ecoregion_SST/Pathfinder_ecoregion_sst.csv", row.names=FALSE)
...
```{r ecoregion monthly SST dataframe construction HadISST}
...
```{r ecoregion monthly SST dataframe construction Pathfinder}

## Load the ecoregion extracted SST data (here using Rdata, but you can use csv if smaller)
eco_sst_path_df <- readRDS("data/Pathfinder/Ecoregion_SST/Pathfinder_ecoregion_sst.rds")
...
eco_sst_path_df <- left_join(eco_sst_path_df, dates, by = c("date" = "date"))
...
```{r}
####################### Reefs
## Coral reefs from 1870-2019 (HadISST; will update this to 2020 once that data are released) — 0.039 per decade
#had_sst <- sst_gam_df
had_full_lm <- lm(sst ~ year, data = had_sst)
had_full_lm

had_full_lm_pred <- data.frame(time = had_sst$year,
                       sst = had_sst$sst,
                       preds = predict(had_full_lm, newdata = had_sst))

## Predicted year means
# 1870: 27.04041
# 2019: 27.62056
# Rate: 0.003894 per year (0.039 per decade)



## Coral reefs from 1990-2019 (HadISST; will update this to 2020 once that data are released) — 
had_sst_sub <- subset(had_sst, year >= 1990)
had_sub_lm <- lm(sst ~ year, data = had_sst_sub)
had_sub_lm

had_sub_lm_pred <- data.frame(time = had_sst_sub$year,
                               sst = had_sst_sub$sst,
                               preds = predict(had_sub_lm, newdata = had_sst_sub))

## Predicted year means
# 1990: 27.34772
# 2019: 27.82927
# Rate: 0.01661 per year (0.166 per decade)



## Coral reefs with Pathfinder from 1982-2019 — 
#path_sst <- sst_gam_df
path_full_lm <- lm(sst ~ year, data = path_sst)
path_full_lm

path_full_lm_pred <- data.frame(time = path_sst$year,
                               sst = path_sst$sst,
                               preds = predict(path_full_lm, newdata = path_sst))

## Predicted year means
# 1982: 26.84132
# 2019: 27.59676
# Rate: 0.02042 per year (0.204 per decade)



## Coral reefs with Pathfinder from 1990-2019 — 
path_sst_sub <- subset(path_sst, year >= 1990)
path_sub_lm <- lm(sst ~ year, data = path_sst_sub)
path_sub_lm

path_sub_lm_pred <- data.frame(time = path_sst_sub$year,
                              sst = path_sst_sub$sst,
                              preds = predict(path_sub_lm, newdata = path_sst_sub))

## Predicted year means
# 1990: 27.00363
# 2019: 27.59715
# Rate: 0.02047 per year (0.205 per decade)


####################### Basin
## Caribbean basin from 1870-2019 (HadISST; will update this to 2020 once that data are released) — 



## Caribbean basin from 1990-2019 (HadISST; will update this to 2020 once that data are released) — 



## Caribbean basin from 1990-2019 with Pathfinder — 




region_names <- levels(factor(ecoregion_sst$ecoregion))

#ecoregion_sst_sub <- subset(ecoregion_sst, year >= 1990)
...
for (region in 1:length(region_names)) {
  ...
  ecoregion_subset <- subset(ecoregion_sst, ecoregion == region_names[region])
  ecoregion_lm <- lm(mean_sst ~ year, data = ecoregion_subset)
  slope <- ecoregion_lm[["coefficients"]][["year"]]
  
  slope_df <- data.frame(ecoregion = region_names[region],
                         slope = (slope*10))
  
  ecoregion_slope <- rbind(ecoregion_slope, slope_df)
  }
...
region_names <- levels(factor(eco_sst_path_df$ecoregion))
ecoregion_warm_year <- data.frame(ecoregion = region_names,
                                  warm_year = c(1986, 1985, 1980, 1986, 1980, 1980, 1981, 1993))
...
for (region in 1:length(region_names)) {
  ...
  ecoregion_subset <- subset(eco_sst_path_df, ecoregion == ecoregion_warm_year[region, 1])
  ecoregion_subset <- subset(ecoregion_subset, year >= ecoregion_warm_year[region, 2])
  ecoregion_lm <- lm(sst ~ year, data = ecoregion_subset)
  slope <- ecoregion_lm[["coefficients"]][["year"]]
  lw_CI <- confint(ecoregion_lm, level = 0.95)[2,1]
  up_CI <- confint(ecoregion_lm, level = 0.95)[2,2]
...
ecoregion_slope$total_warm <- ((ecoregion_slope$slope / 10) * (2019 - ecoregion_slope$warm_year))

ggplot(data = ecoregion_slope, aes(x = ecoregion, y = slope, colour = warm_year)) +
  theme_cowplot() +
  geom_linerange(aes(ymin = lw_CI, ymax = up_CI)) +
  geom_point()
