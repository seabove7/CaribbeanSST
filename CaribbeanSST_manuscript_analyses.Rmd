...
# install.packages("sf", "ncdf4", "raster", "ggrepel", "maptools", "rgdal", "ggplot2", "tidyverse", "xts", "RColorBrewer", "tidyr", "dplyr", "fields", "gganimate", "sp", "plotly", "grDevices", "repmis", "binr", "cowplot", "viridis")
...
hadplot_markdown <- ggplot() +
  ...
  theme(legend.position = c(0.05, 0.9), legend.key.size = unit(0.3, "cm"), legend.text = element_text(size = 7), legend.background = element_rect(fill = NA)) + # legend theme
  ...
  scale_colour_gradient2("", low = "#2166ac", mid = "#d1e5f0", high = "#d6604d", midpoint = mean(had_full2$year), space="Lab") +
...
plot_grid(hadplot_markdown, HadISST_SST_plot, ncol = 1, labels = c("A", "B"), align = "hv") +
  ggsave("figures/Manuscript_figs/Fig3_SST_trends2.pdf", width = 6, height = 7.5)
...
## calculate summary statistic per month across years (currently, this is not being included in this plot: see the geom_ribbon/geom_line lines in plot)
...
path_full2$year <- as.integer(path_full2$year) # make year integer for better plotting
...
ggplot() +
  theme_bw() +
  theme(panel.grid.major=element_blank(), panel.grid.minor=element_blank(), panel.background=element_blank(), strip.background = element_blank(), panel.border = element_rect(colour = "black", fill=NA, size=1)) +
  geom_hline(yintercept = mean(Path_year_avg$mean), colour = "grey", linetype = 2) +
  geom_ribbon(data = Path_year_avg, aes(x = year, ymin = lowerci, ymax = upperci), colour = "darkgrey", alpha = 0.3, linetype = 1) +
  #geom_line(data = Path_year_avg, aes(x = year, y = mean), colour = "black", group = 1, size = 1.2) +
  scale_x_continuous(name = "Year", breaks = c(1880, 1900, 1920, 1940, 1960, 1980, 2000, 2019)) +
  ylab('SST (°C)') +
  scale_y_continuous(breaks = seq(24, 30, 1)) +
  geom_line(data = Path_year_avg, aes(x = year, y = max), colour = "#d6604d", group = 1, size = 0.5) +
  geom_line(data = Path_year_avg, aes(x = year, y = min), colour = "#2166ac", group = 1, size = 0.5) + 
  geom_smooth(data = Path_year_avg, method = "gam", aes(x = year, y = mean), colour = "black", group = 1, size = 1, se = FALSE) + # smoothed mean temp
  ggtitle("Pathfinder (1981 - 2019)") +
  coord_cartesian(xlim = c(1870, 2019)) +
...
combo_plot <- ggplot() +
  ...
  geom_ribbon(data = HADI_year_avg[-151,], aes(x = year, ymin = lowerci, ymax = upperci), colour = "darkgrey", alpha = 0.1, linetype = 1) + 
  geom_line(data = HADI_year_avg[-151,], aes(x = year, y = max), colour = "#d6604d", group = 1, size = 0.5, alpha = 0.5) +
  geom_line(data = HADI_year_avg[-151,], aes(x = year, y = min), colour = "#2166ac", group = 1, size = 0.5, alpha = 0.5) + 
  geom_line(data = HADI_year_avg[-151,], aes(x = year, y = mean), colour = "darkgrey", group = 1, size = 0.5, alpha = 0.5) + 
  geom_smooth(data = HADI_year_avg[-151,], method = "gam", aes(x = year, y = mean), colour = "darkgrey", group = 1, size = 1, se = FALSE, alpha = 0.5) +
  # add the Pathfinder dataset on top
  geom_hline(yintercept = mean(Path_year_avg[-151,]$mean), colour = "black", linetype = 2) +
  ...
  geom_line(data = Path_year_avg[-151,], aes(x = year, y = mean), colour = "black", group = 1, size = 0.5) + 
  geom_smooth(data = Path_year_avg[-151,], method = "gam", aes(x = year, y = mean), colour = "black", group = 1, size = 1, se = FALSE) +
  ggtitle("Caribbean coral reefs historic SST trends (1981 - 2019)") +
  scale_y_continuous(breaks = seq(24, 30, 1)) +
  coord_cartesian(xlim = c(1870, 2019))
  #ggsave("figures/Combined_SST_annual.pdf", width = 6, height = 4) # I have not saveed this figure outside the Rmarkdown file
...
had_map <- ggplot() +  
  ...
  ylab("Latitude") +
  xlab("Longitude") +
  ggtitle(paste0("SST slope from ", format.Date(start_date, "%Y"), " to ", format.Date(end_date, "%Y")," (HadISST)"))
...
```{r ecoregion map, message=FALSE, warning=FALSE}
...
#regions_cropped <- crop(region, extent(-100, -55, 8, 30)) # crop for the Caribbean region only
...
## Read in the clipped Caribbean ecoregion shapefile
ecoreg <- read_sf("data/EcoRegions/Caribbean_ecoregions.shp")

## Subset the shapefile further for the TNA province and remove Bermuda and SGoM ecoregions
carib_ecoreg <- ecoreg %>% 
  filter(PROVINCE =="Tropical Northwestern Atlantic" & ECOREGION != "Bermuda" & ECOREGION != "Southern Gulf of Mexico") 

## Plot the ecoregions on the Caribbean map
ecoreg_map <- ggplot(carib_ecoreg) + 
  geom_sf(colour = "black", aes(fill = ECOREGION), alpha = 0.5) +
  scale_fill_brewer("Ecoregion", palette = "Set2") +
  theme_bw() +
  theme(panel.grid.major=element_blank(), panel.grid.minor=element_blank(), panel.background=element_blank(), strip.background = element_blank(), panel.border = element_rect(colour = "black", fill=NA, size=1)) +
  geom_polygon(data = land, aes(x=long, y = lat, group = group), fill = "grey70", color='grey39', lwd = 0.1) +
  #geom_point(data = reef_pt, aes( x = X, y = Y), colour = "black", size = 0.1) +
...
```{r subsetting reefs by ecoregion, message=FALSE, warning=FALSE}

### Subset reefs by ecoregions for analyses by region

# reef shape files containing reef polygons
ecoregions2 <- readOGR('data/EcoRegions', 'Caribbean_ecoregions') 

# Select the only the desired ecoregion:
Bahamian_ecoreg <- ecoregions2[ecoregions2@data$ECOREGION == "Bahamian", ]
ECarib_ecoreg <- ecoregions2[ecoregions2@data$ECOREGION == "Eastern Caribbean", ]
FL_ecoreg <- ecoregions2[ecoregions2@data$ECOREGION == "Floridian", ]
Antilles_ecoreg <- ecoregions2[ecoregions2@data$ECOREGION == "Greater Antilles", ]
SCarib_ecoreg <- ecoregions2[ecoregions2@data$ECOREGION == "Southern Caribbean", ]
SWCarib_ecoreg <- ecoregions2[ecoregions2@data$ECOREGION == "Southwestern Caribbean", ]
WCarib_ecoreg <- ecoregions2[ecoregions2@data$ECOREGION == "Western Caribbean", ]


# Clip the reefs by the selected ecoregion bounds:
Bahamian_reefs <- shapes[Bahamian_ecoreg,]
ECarib_reefs <- shapes[ECarib_ecoreg,]
FL_reefs <- shapes[FL_ecoreg,]
Antilles_reefs <- shapes[Antilles_ecoreg,]
SCarib_reefs <- shapes[SCarib_ecoreg,]
SWCarib_reefs <- shapes[SWCarib_ecoreg,]
WCarib_reefs <- shapes[WCarib_ecoreg,]

```

```{r ecoregion reef plot}

# PLot the reefs as points, coloured by region
eco_reef_map <- ggplot() +
    #geom_rect(aes(xmin=-Inf,xmax=Inf,ymin=-Inf, ymax=41.5), fill="#666666") +
    coord_sf(xlim = c(-95,-55), ylim = c(8,33)) +
    ylab("Latitude") +
    xlab("Longitude") +
    theme_bw() +
    theme(panel.grid.major=element_blank(), panel.grid.minor=element_blank(), panel.background=element_blank(), strip.background = element_blank(), panel.border = element_rect(colour = "black", fill=NA, size=1)) +
    geom_point(data = Bahamian_reefs, aes(x=long, y = lat, group = group), fill = "#1B9E77", color='#1B9E77', size = 0.2) +
    geom_point(data = ECarib_reefs, aes(x=long, y = lat, group = group), fill = "#D95F02", color='#D95F02', size = 0.2) +
    geom_point(data = FL_reefs, aes(x=long, y = lat, group = group), fill = "#7570B3", color='#7570B3', size = 0.2) +
    geom_point(data = Antilles_reefs, aes(x=long, y = lat, group = group), fill = "#E7298A", color='#E7298A', size = 0.2) +
    geom_point(data = SCarib_reefs, aes(x=long, y = lat, group = group), fill = "#66A61E", color='#66A61E', size = 0.2) +
    geom_point(data = SWCarib_reefs, aes(x=long, y = lat, group = group), fill = "#E6AB02", color='#E6AB02', size = 0.2) +
    geom_point(data = WCarib_reefs, aes(x=long, y = lat, group = group), fill = "#A6761D", color='#A6761D', size = 0.2) +
    geom_polygon(data = land, aes(x=long, y = lat, group = group), fill = "grey70", color='grey39', lwd = 0.1)


# Plot actual shape polygons, coloured by region
#ggplot() +
#    geom_rect(aes(xmin=-Inf,xmax=Inf,ymin=-Inf, ymax=41.5), fill="#666666") +
#    geom_polygon(data = land, aes(x=long, y = lat, group = group), fill = "lightgrey", color='lightgrey', lwd = 0.1) +
#    coord_sf(xlim = c(-90,-57), ylim = c(8,28)) +
#    ylab("Latitude") +
#    xlab("Longitude") +
#    theme_bw() +
#    geom_polygon(data = Bahamian_reefs, aes(x=long, y = lat, group = group), fill = "#1B9E77", color='#1B9E77', lwd = 0.1) +
#    geom_polygon(data = ECarib_reefs, aes(x=long, y = lat, group = group), fill = "#D95F02", color='#D95F02', lwd = 0.1) +
#    geom_polygon(data = FL_reefs, aes(x=long, y = lat, group = group), fill = "#7570B3", color='#7570B3', lwd = 0.1) +
#    geom_polygon(data = Antilles_reefs, aes(x=long, y = lat, group = group), fill = "#E7298A", color='#E7298A', lwd = 0.1) +
#    geom_polygon(data = SCarib_reefs, aes(x=long, y = lat, group = group), fill = "#66A61E", color='#66A61E', lwd = 0.1) +
#    geom_polygon(data = SWCarib_reefs, aes(x=long, y = lat, group = group), fill = "#E6AB02", color='#E6AB02', lwd = 0.1) +
#    geom_polygon(data = WCarib_reefs, aes(x=long, y = lat, group = group), fill = "#A6761D", color='#A6761D', lwd = 0.1) 

```

```{r combined ecoregion map}

plot_grid(ecoreg_map, eco_reef_map, ncol = 1, labels = c("A", "B")) +
  ggsave("figures/Fig14_Ecoregions_map.pdf", width = 6, height = 5)
...
```{r ecoregion SST extraction - REMOTE, eval=FALSE, include=FALSE}

# ***NOTE***
####--- this portion of the script was run on a high performance computing cluster because of the long runtime needed
####--- see separate R script (Ecoregion_SSTExtraction.R) for instructions (located in the code folder)
####--- the script can be put on a cluster with the HadISST netCDF, Caribbean reef shapefile, and ecoregion shapefile to run


##########################################
# If working with a smaller data set, you can run the below code to accomplish the same outcome as the Ecoregion_SSTExtraction.R script.
# The code below has been adapted to specifically work with the HadISST_sst.nc, Caribbean_ecoregions.shp, and Caribbean_reefs.shp files:

## Identify the ecoregion you want to extraact reef SST for (you can run 'ecoregions2@data[["ECOREGION"]]' to list ecoregion names)
region <- "Bahamian" # specify the name of the data set here
...
raster_brick <- brick(HadISST_sst_path)
...
## for loop that runs through each raster layer and extracts the SST corresponding to reef polygons
for (i in 1:3) { #raster_brick@file@nbands
...
```{r ecoregion SST GPS extraction, eval=FALSE, include=FALSE}

## Read in the individual ecoregion extracted SST data
BA_sst <- read.csv("data/HadISST/Ecoregion_SST/Bahamian_reefs.csv") # no NA values for polygons so it is ready!
EC_sst <- read.csv("data/HadISST/Ecoregion_SST/ECarib_reefs.csv") # no NA values for polygons so it is ready!
FL_sst <- read.csv("data/HadISST/Ecoregion_SST/FL_reefs.csv") # has some NA values so those need GPS extraction
SW_sst <- read.csv("data/HadISST/Ecoregion_SST/SWCarib_reefs.csv") # has some NA values so those need GPS extraction
WC_sst <- read.csv("data/HadISST/Ecoregion_SST/WCarib_reefs.csv") # has some NA values so those need GPS extraction
SC_sst <- read.csv("data/HadISST/Ecoregion_SST/SCarib_reefs.csv") # has some NA values so those need GPS extraction
AN_sst <- read.csv("data/HadISST/Ecoregion_SST/Antilles_reefs.csv") # has some NA values so those need GPS extraction


### This portion of code needs to be run for the forllowing ecoregions that were missing SST values for some polygons:
## "Floridian" "Greater Antilles" "Southern Caribbean" "Southwestern Caribbean" "Western Caribbean"

## Select the ecoregion to extract SST by GPS coordinates
region_name <- "Greater Antilles" # input desired ecoregion here
region <- AN_sst
region_reef <- Antilles_reefs

## selected ecoregion dataframe modification
region_poly <- region[,c(1,8,7,5)] # select only the polygon ID, ecoregion, weighted mean SST, and time 
region_poly$polygon <- factor(region_poly$polygon)
region_poly <- spread(region_poly, polygon, weighted) # convert from long to wide format


## create dataframe of missing polygon GPS corrdinates to extract from HadISST raster
NAcol <- colnames(region_poly)[colSums(is.na(region_poly)) > 0] # identify the columns with NA values to pull SST from GPS points instead of polygons

## Convert ecoregion reefs spatial object to simple feature and extract GSP coordinates of polygons
region_reef_sf <- st_as_sf(region_reef)
region_reef_pt <- st_coordinates(region_reef_sf) # extracts all the polygon coordinates into a matrix
region_reef_pt <- as.data.frame(region_reef_pt) # convert from matrix to dataframe
region_reef_gps <- region_reef_pt %>% filter(L3 == NAcol) # select only GPS points of polygons missing SST data above

## converting sampling points to spatial points and applying projection to points
LongLat <- cbind(region_reef_gps$X, region_reef_gps$Y) 
eco_ReefPTS <- SpatialPoints(LongLat)
proj <- "+proj=longlat +ellps=WGS84 +datum=WGS84"
projection(eco_ReefPTS) <- proj


#some pixels fall on land. Finding nearest valid pixels
#pixels coordinates
eco_brick <- brick(HadISST_sst_path)
first_day <- eco_brick[[1]]
pixel_coordinates <- coordinates(first_day)
#delete coordinates where data is missing (NA)
#first, extract data for all pixels
data <- raster::extract(first_day, pixel_coordinates)
...
#create new sampling points, shifted to the center of the pixel they fall on, or to the near pixel in case that one is NA
eco_ReefPTS_shifted <- vector()
#loop cycling through sampling points
for (p in 1:length(eco_ReefPTS)) {
  #geodesic distances
  distances <- rdist.earth(coordinates(eco_ReefPTS[p]),pixel_coordinates,miles=FALSE)
  #coordinates of the pixel that is at shorter distance 
  shifted_coordinates <- pixel_coordinates[which.min(distances),]
  #assigning those coordinates to a new item
  cmd <- paste0("sp.",p,"<-shifted_coordinates")
  eval(parse(text=cmd))
  #points(pt[1],pt[2])
  cmd <- paste0("eco_ReefPTS_shifted<-rbind(eco_ReefPTS_shifted,sp.",p,")")
  eval(parse(text = cmd))
}
...
# extract data from the sst brick with the sampling points shifted
eco_sst <- raster::extract(eco_brick, eco_ReefPTS_shifted)


# load in the time data again for HadISST and modify to selected range
load("data/HadISST/HadISST_times.Rdata")
Tstart_index <- min(which(difftime(nc_times,start_date)>0))
Tend_index <- min(which(difftime(nc_times,"2020-04-16")>0))
time <- nc_times[Tstart_index:Tend_index]

# convert the data matrix to a xts object
eco_sst_xts <- xts(t(eco_sst), time) #t is used to transpose the matrix because xts assumes dates are along the lines, not columns
colnames(eco_sst_xts) <- region_reef_gps$L3 #ID the columns

eco_sst_df <- data.frame(date=index(t(eco_sst_xts)), coredata(t(eco_sst_xts)))
eco_sst_df$polygon <- factor(region_reef_gps$L3) #ID the columns
eco_sst_df$date <- NULL

eco_sst_long <- gather(eco_sst_df, date, sst, X1870.01.16.15.00.00:X2020.04.16.15.00.00)


eco_gps <- aggregate(eco_sst_long, by = list(eco_sst_long$date, eco_sst_long$polygon), FUN = mean)[,c(1:2,5)]
eco_gps2 <- eco_gps %>% separate(Group.1, c("year", "month", "day", "hour", "min", "sec")) # create year, month, and day column columns
eco_gps2$date <- paste(eco_gps2$year, eco_gps2$month, eco_gps2$day, sep = ".")

eco_gps2 <- eco_gps2[, c(7,9,8)]
eco_gps2 <- spread(eco_gps2, Group.2, sst)


region_poly <- select(region_poly, -c(NAcol)) # remove the NA columns before adding them back in below

eco_full <- region_poly %>% 
  left_join(eco_gps2, by = c("date" = "date"))


write.csv(eco_full, file = paste("data/HadISST/Ecoregion_SST/", gsub(" ", "", region_name, fixed = TRUE), "_reefs_gps.csv", sep = ""), row.names=FALSE)
...
## Read in the individual ecoregion extracted SST data (these are the full df when GPS extraction needed to happen)
BA_sst <- read.csv("data/HadISST/Ecoregion_SST/Bahamian_reefs.csv")
EC_sst <- read.csv("data/HadISST/Ecoregion_SST/ECarib_reefs.csv")
FL_sst <- read.csv("data/HadISST/Ecoregion_SST/Floridian_reefs_gps.csv") 
SW_sst <- read.csv("data/HadISST/Ecoregion_SST/SouthwesternCaribbean_reefs_gps.csv")
WC_sst <- read.csv("data/HadISST/Ecoregion_SST/WesternCaribbean_reefs_gps.csv")
SC_sst <- read.csv("data/HadISST/Ecoregion_SST/SouthernCaribbean_reefs_gps.csv")
AN_sst <- read.csv("data/HadISST/Ecoregion_SST/GreaterAntilles_reefs_gps.csv")


## Adjust BA and EC from long to wide to match others
BA_sst <- BA_sst[,c(1,8,7,5)] # select only the polygon ID, ecoregion, weighted mean SST, and time 
EC_sst <- EC_sst[,c(1,8,7,5)]


FL_sst <- gather(FL_sst, polygon, weighted, X14:X27)
SW_sst <- gather(SW_sst, polygon, weighted, X1:X55)
WC_sst <- gather(WC_sst, polygon, weighted, X1:X73)
SC_sst <- gather(SC_sst, polygon, weighted, X1:X169)
AN_sst <- gather(AN_sst, polygon, weighted, X1:X184)

## Combine all ecoregion SST dataframes into one and modify for plotting
ecoregion_sst <- rbind(BA_sst, EC_sst, FL_sst, SW_sst, WC_sst, SC_sst, AN_sst)
ecoregion_sst$polygon <- as.factor(gsub("\\X", "", ecoregion_sst$polygon)) # convert the year column to numerics and remove 'X'
#ecoregion_sst$polygon2 <- paste(ecoregion_sst$polygon, ecoregion_sst$ecoregion, sep = "_")
#ecoregion_sst$polygon <- NULL
ecoregion_sst2 <- spread(ecoregion_sst, polygon, weighted) # convert from long to wide format



## create a new dataframe of simple stats per time
eco_sst_full <- data.frame("date" = ecoregion_sst2[,1],
                           "ecoregion" = ecoregion_sst2[,2],
                           "mean_temp" = rowMeans(ecoregion_sst2[,-c(1:2)], na.rm = TRUE),
                           "max_temp" = apply(ecoregion_sst2[,-c(1:2)], 1, FUN=max, na.rm = TRUE),
                           "min_temp" = apply(ecoregion_sst2[,-c(1:2)], 1, FUN=min, na.rm = TRUE))

eco_sst_full <- eco_sst_full %>% separate(date, c("year", "month", "day"), convert = TRUE) # create year, month, and day column columns
eco_sst_full$year <- as.numeric(gsub("\\X", "", eco_sst_full$year)) # convert the year column to numerics and remove 'X'
eco_sst_full$month2 <- factor(eco_sst_full$month) # make a month as a factor column for plotting
...
# Create overall mean SST per ecoregion for plotting
eco_month_avg_mean <- eco_sst_full %>% 
  group_by(ecoregion) %>% 
  summarize(max = round(max(mean_temp, na.rm = TRUE), 2),
            min = round(min(mean_temp, na.rm = TRUE), 2))


# Add '(mean SST: XX)' to each ecoregion label
labs <- paste(levels(eco_month_avg_mean$ecoregion), " (max = ", eco_month_avg_mean$max, "; min = ", eco_month_avg_mean$min, "°C)", sep = "")

# Name existing facet labels based on new labels for plot 
facet_labs <- c("Bahamian" = labs[1], "Eastern Caribbean" = labs[2], "Floridian" = labs[3], "Southwestern Caribbean" = labs[4], "Western Caribbean" = labs[5], "Southern Caribbean" = labs[6], "Greater Antilles" = labs[7])


#### Data visualization of monthly SST per year #### 

# make year integer for better plotting
eco_sst_full$year <- as.integer(eco_sst_full$year)
...
ggplot() +
  ...
  geom_line(data = eco_sst_full, aes(x = month, y = mean_temp, group = year, colour = year), alpha = 0.6, size = 0.2) +
  scale_colour_gradient2("Year", low = "#2166ac", mid = "#d1e5f0", high = "#d6604d", midpoint = mean(eco_sst_full$year), space="Lab") +
  ...
  scale_y_continuous(breaks = seq(22, 32, 1)) +
  ylab('SST (°C)') +
  facet_wrap(~ecoregion, ncol = 2, labeller = labeller(ecoregion = facet_labs)) +
  guides(colour = guide_colorbar(frame.colour = "black", ticks.colour = "black")) +
...
# Subset out 2020 data 
eco_sst_full <- subset(eco_sst_full, year != "2020")


# Calculate summary statistic per month across years
ecoregion_year_avg <- eco_sst_full %>% 
...
# Create overall mean SST per ecoregion for plotting
ecoregion_year_mean <- eco_sst_full %>% 
  group_by(ecoregion) %>% 
  summarize(mean = round(mean(mean_temp, na.rm = TRUE), 2),
            range = round(mean((max(mean_temp, na.rm = TRUE) - min(mean_temp, na.rm = TRUE)), na.rm = TRUE), 2))



# Add '(mean SST: XX)' to each ecoregion label
labels <- paste(levels(ecoregion_year_mean$ecoregion), " (mean: ", ecoregion_year_mean$mean, "; range: ", ecoregion_year_mean$range, "°C)", sep = "")

# Name existing facet labels based on new labels for plot 
facet_labels <- c("Bahamian" = labels[1], "Eastern Caribbean" = labels[2], "Floridian" = labels[3], "Southwestern Caribbean" = labels[4], "Western Caribbean" = labels[5], "Southern Caribbean" = labels[6], "Greater Antilles" = labels[7])
...
ggplot() +
  theme_bw() +
  theme(panel.grid.major=element_blank(), panel.grid.minor=element_blank(), panel.background=element_blank(), strip.background = element_blank(), panel.border = element_rect(colour = "black", fill=NA, size=1), axis.text.x = element_text(angle = 30, hjust = 0.8)) +
  #geom_point(data = ecoregion_year_mean, aes(x = 2019, y = mean), colour = "white") +  
  geom_segment(data = ecoregion_year_avg, aes(y = mean(mean), yend = mean(mean), x = 1870, xend = 2019), colour = "grey", linetype = 2) +
  #geom_label_repel(data = ecoregion_year_mean, aes(x = 2019, y = mean, label = mean), nudge_y = -5.6, nudge_x = 3, label.size = NA, size = 2.7, segment.size = 0.3) +
  geom_ribbon(data = ecoregion_year_avg, aes(x = year, ymin = lowerci, ymax = upperci), colour = "darkgrey", alpha = 0.3, linetype = 1) + 
  #geom_line(data = ecoregion_year_avg, aes(x = year, y = mean), colour = "black", group = 1, size = 1.2) +
  scale_x_continuous(name = "", breaks = seq(1880, 2019, 20)) +
  ylab('SST (°C)') +
  geom_line(data = ecoregion_year_avg, aes(x = year, y = max), colour = "#d6604d", group = 1, size = 0.5) +
  geom_line(data = ecoregion_year_avg, aes(x = year, y = min), colour = "#2166ac", group = 1, size = 0.5) + 
  geom_smooth(data = ecoregion_year_avg, method = "gam", aes(x = year, y = mean), colour = "black", group = 1, size = 0.5, se = FALSE, formula = y ~ s(x, bs = "cs")) + # smoothed mean temp
  coord_cartesian(xlim = c(1870, 2019)) +
  scale_y_continuous(breaks = seq(22, 32, 2)) +
  facet_wrap(~ ecoregion, ncol = 2, labeller = labeller(ecoregion = facet_labels)) +
  ggsave("figures/Fig16_Ecoregion_SST_annual.pdf", width = 7, height = 8)
...
THIS IS THE WORKING CHUNK HERE:
...
## Subset the shapefile further for the TNA province and remove Bermuda and SGoM ecoregions
carib_ecoreg <- ecoreg %>% 
  filter(PROVINCE =="Tropical Northwestern Atlantic" & ECOREGION != "Bermuda") 
...
ggplot(carib_ecoreg) + 
  ...
  scale_fill_brewer("Ecoregion", palette = "Set2") +
  theme_bw() +
  geom_point(data = reef_gps, aes(x = lon, y = lat), colour = "blue", size = 0.5) +
  theme(panel.grid.major=element_blank(), panel.grid.minor=element_blank(), panel.background=element_blank(), strip.background = element_blank(), panel.border = element_rect(colour = "black", fill=NA, size=1)) +
  geom_polygon(data = land, aes(x=long, y = lat, group = group), fill = "grey70", color='grey39', lwd = 0.1) +
  #geom_point(data = reef_pt, aes( x = X, y = Y), colour = "black", size = 0.1) +
  coord_sf(xlim = c(-95,-55), ylim = c(8,33)) +
...
for(e in 1:length(eco_list)) {
  ecoregion_name <- eco_list[[e]]
  subset_ecoreg <- ecoregions2[ecoregions2@data$ECOREGION == ecoregion_name, ]
  projection(subset_ecoreg) <- proj
  ecoregion_reefs <- reef_points[subset_ecoreg, ]
  
  ecoregion_df <- as.data.frame(ecoregion_reefs)
  ecoregion_df$reef_id <- factor(ecoregion_df$reef_id)
  ecoregion_df$ecoregion <- rep(ecoregion_name, length(ecoregion_df$reef_id)) 
  ecoregion_df$n <- length(ecoregion_df$reef_id)
  
  compiled_eco_df <- rbind(compiled_eco_df, ecoregion_df) 
}




#### Things I am testing from Laura's code:
...
ggplot() + 
  geom_point(data = compiled_eco_df, aes(x = lon, y = lat, colour = ecoregion), size = 0.5) +
  geom_sf(data = world) +
  theme_bw() +
  theme(panel.grid.major=element_blank(), panel.grid.minor=element_blank(), panel.background=element_blank(), strip.background = element_blank(), panel.border = element_rect(colour = "black", fill=NA, size=1)) +
